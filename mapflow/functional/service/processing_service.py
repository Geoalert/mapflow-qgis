...
class ProcessingService(QObject):
    ...
    def __init__(self,
        ...
        self.processings = {}
        #: The project's templates. Filled by this service's own project fetch and merged into its
        #: rows, which is why it stays here while the in-template view lives in `TemplateService`.
        self.templates = {}
    ...
    def _apply_poll_timer_state(self):
        ...
        Only ever reached from the project fetch's callback, so it needs no in-template guard: the
        in-template view sets its own cadence on enter and never lands here."""
    ...
    def all_selected_templates_editable(self) -> bool:
        """True when the selection is non-empty, is templates only (no processings), and every
        selected template is editable by the current user (``can_edit_template``). Used to gate
        the Delete button for a contributor, who may delete their own templates but never a
        processing."""
        if not self.is_only_templates_selected():
            return False
        return all(self.app_context.can_edit_template(t) for t in self.selected_templates())

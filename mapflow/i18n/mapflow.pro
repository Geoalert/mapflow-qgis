FORM_DIRECTORY = ../dialogs/static/ui

FORMS = $$FORM_DIRECTORY/error_message.ui\
        $$FORM_DIRECTORY/image_dialog.ui\
        $$FORM_DIRECTORY/login_dialog.ui\
        $$FORM_DIRECTORY/main_dialog.ui\
        $$FORM_DIRECTORY/mosaic_dialog.ui\
        $$FORM_DIRECTORY/processing_details.ui\
        $$FORM_DIRECTORY/processing_dialog.ui\
        $$FORM_DIRECTORY/processing_start_confirmation.ui\
        $$FORM_DIRECTORY/project_dialog.ui\
        $$FORM_DIRECTORY/provider_dialog.ui\
        $$FORM_DIRECTORY/raster_layers_dialog.ui\
        $$FORM_DIRECTORY/review_dialog.ui
         

SOURCES = ../mapflow.py\
          ../config.py\
          ../dialogs/confirm_processing_start_dialog.py\
          ../dialogs/error_message_widget.py\
          ../dialogs/image_dialog.py\
          ../dialogs/login_dialog.py\
          ../dialogs/main_dialog.py\
          ../dialogs/mosaic_dialog.py\
          ../dialogs/processing_details_dialog.py\
          ../dialogs/processing_dialog.py\
          ../dialogs/project_dialog.py\
          ../dialogs/provider_dialog.py\
          ../dialogs/review_dialog.py\
          ../dialogs/select_aoi_layers_dialog.py\
          ../dialogs/upload_raster_layer_dialog.py\
          ../errors/api_errors.py\
          ../errors/data_errors.py\
          ../errors/error_message_list.py\
          ../errors/errors.py\
          ../errors/processing_errors.py\
          ../functional/api/data_catalog_api.py\
          ../functional/api/processing_api.py\
          ../functional/api/project_api.py\
          ../functional/controller/processing_controller.py\
          ../functional/controller/project_processing_controller.py\
          ../functional/service/alert_service.py\
          ../functional/service/aoi_service.py\
          ../functional/service/area_calculator_service.py\
          ../functional/service/data_catalog.py\
          ../functional/service/processing_service.py\
          ../functional/service/project_service.py\
          ../functional/service/provider_service.py\
          ../functional/view/aoi_view.py\
          ../functional/view/data_catalog_view.py\
          ../functional/view/processing_view.py\
          ../functional/view/project_view.py\
          ../functional/helpers.py
          
TRANSLATIONS = mapflow_ru.ts mapflow_zh.ts mapflow_es.ts mapflow_pt.ts
